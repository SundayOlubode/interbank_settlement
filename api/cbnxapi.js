...
app.post("/api/netting/bilateral", async (req, res) => {
  ...
  try {
    ...
    console.log(`================= Bilateral Netting =======================`);
    console.log(`Banks: ${BankA} & ${BankB}`);
    console.log(
      "Bilateral netting offset amount: ",
      payload.offset.toLocaleString(), "eNaira"
    );
    console.log(`Number of txs offsets: `, payload.updates.length);
    console.log("===========================================================");
...
app.post("/api/netting/multilateral", async (req, res) => {
  try {
    ...
    const calcBytes = await contract.evaluateTransaction(
      "CalculateMultilateralOffset"
    );
